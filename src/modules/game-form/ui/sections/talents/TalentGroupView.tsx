...
import type { TalentGroup as TalentGroupType } from "./utils/group-talents-by-branch.utils.ts";
...
export default function TalentGroupView({ group, onDelete }: Props) {
  return (
    <Box sx={{ mb: 3 }}>
      {/* Заголовок группы */}
      <Typography
      ...
      <Grid container gap={2}>
        {group.talents.map((talent) => (
          ...
          >
            ...
            >
              <IconButton
                onClick={() => onDelete(talent.fieldIndex, talent.name)}
            ...
            <Grid container wrap={"wrap"}>
              <Grid size={{ xs: 12, md: 8 }}>
                <BaseField
                  fieldName={`talents.list.${talent.fieldIndex}.name`}
                  orientation="row"
                  fieldType="text"
                />
              </Grid>
              <Grid size={{ xs: 8, md: 4 }}>
                <BaseField
                  fieldName={`talents.list.${talent.fieldIndex}.branch`}
                  label={{
                    text: "Ветка",
                    color: "secondary",
                    ...LABEL_STYLES,
              ...
              <Grid size={{ xs: 12, md: 10 }} order={{ xs: 4, md: 3 }}>
                <TooltipWrapper text={talent.effect}>
                  <BaseField
                    fieldName={`talents.list.${talent.fieldIndex}.effect`}
                    orientation="row"
                    fieldType="text"
              ...
              <Grid size={{ xs: 4, md: 2 }} order={{ xs: 3, md: 4 }}>
                <BaseField
                  fieldName={`talents.list.${talent.fieldIndex}.rang`}
                  label={{
                    text: "Ранг",
                    color: "secondary",
                  }}
